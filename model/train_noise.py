...
for epoch in range(1, EPOCHS + 1):
    ...
    for batch_idx, (x, y) in progress:
        ...
        # 🔥 온라인 증강 (batch 안 각 세그먼트에 독립적으로 적용)
        with torch.no_grad():
            B = x.size(0)
    ...
    # validation에서는 noise만 사용하고 geometry 증강은 끔
    ...
    print(f"\n[{epoch:02d}] Epoch complete | avg_loss={avg_loss:.4f} | val_f1(noisy)={f1:.4f} | lr={current_lr:.2e}")

    if f1 > best_f1:
        best_f1 = f1
        torch.save({"model": model.state_dict(), "label_map": label_map}, "chk/best_noise_val.pt")
        print(f"  ✅ New best model saved (Noisy Val F1={best_f1:.4f})")
