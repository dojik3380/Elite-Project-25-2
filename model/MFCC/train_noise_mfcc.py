...
#    - DroneDataset는 이미 SEGMENT_SEC (ex. 2초) 단위로 segment화 되어 있으므로
#      각 segment 하나당 SEGMENT_SEC 초로 duration 집계
# ============================================================
...
for i in range(len(train_ds)):
    ...
    duration_sum[label_idx] += SEGMENT_SEC  # segment = SEGMENT_SEC 초

# weight = 1 / sqrt(total_seconds)
class_weights_list = []
for cls_idx in range(n_classes):
    tot_sec = duration_sum[cls_idx]
    if tot_sec == 0:
        w = 1.0
    else:
        w = 1.0 / np.sqrt(tot_sec)
...
# ============================================================
# 3) Online Augment 설정
#    - train: geometry + noise 모두 ON
#    - val:   geometry OFF, noise만 ON (노이즈 환경 성능 측정용)
# ============================================================
augment = WaveAugmentV1(
    sample_rate=TARGET_SR,
    use_augment=True,   # 기본값: geometry 증강 ON
    use_noise=True      # 기본값: noise 증강 ON
)
...
for epoch in range(1, EPOCHS + 1):
    ...
    # 🔥 Train에서는 geometry + noise 모두 사용
    augment.set_augment(True)
    augment.set_noise(True)
    ...
    for x, y in progress:
        ...
        with torch.no_grad():
            ...
            for i in range(B):
                x_aug[i] = augment(x[i])   # [1, T] 단위로 증강
            x = x_aug
    ...
    # ============================================================
    # 6) Validation (🔥 noise-only validation)
    #     - geometry 증강 OFF
    #     - noise ON
    #     → 실제 noise 환경에서의 robustness 평가
    # ============================================================
    model.eval()
    y_true, y_pred = [], []

    augment.set_augment(False)   # gain/shift/stretch/mask OFF
    augment.set_noise(True)      # noise ON

    with torch.no_grad():
        for x, y in val_loader:
            x = x.to(DEVICE)

            # 🔥 validation용 noisy 입력 생성
            B = x.size(0)
            x_aug = torch.empty_like(x)
            for i in range(B):
                x_aug[i] = augment(x[i])
            ...
            pred = torch.argmax(model(x_noisy), dim=1)
    ...
    print(f"[{epoch:02d}] loss={running_loss/len(train_loader):.4f} | val_f1(noisy)={f1:.4f}")

    if f1 > best_f1:
        best_f1 = f1
        torch.save(
            {"model": model.state_dict(), "label_map": label_map},
            "chk/mfcc_best_noise.pt"
        )
        print(f"🔥 BEST model updated! (Noisy Val F1={best_f1:.4f})")

print("\n🎉 MFCC Noise-Robust Training Completed.")
