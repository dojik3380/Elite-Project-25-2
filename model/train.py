...
class DummyTorchCodec(ModuleType):
    __spec__ = DummySpec()
    __file__ = __file__
    __path__ = []
...
_dummy_tc = DummyTorchCodec('torchcodec')
sys.modules['torchcodec'] = _dummy_tc
sys.modules['torchcodec.decoders'] = _dummy_tc
sys.modules['torchcodec.samplers'] = _dummy_tc
sys.modules['datasets.features._torchcodec'] = _dummy_tc
...
def main():
    ...
    print(f"Class balance: non-drone={n0}, drone={n1}, pos_weight={pos_weight.item():.3f}")
    ...
    print(f"Starting training: {args.epochs} epochs, {total_steps} steps/epoch")
